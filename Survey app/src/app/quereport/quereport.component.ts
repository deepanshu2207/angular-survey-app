...
@Component({
  selector: 'app-quereport',
  templateUrl: './quereport.component.html',
  styleUrls: ['./quereport.component.css']
})
export class QuereportComponent implements OnInit {
...
getques(){
  ...
  this.reportservice.allquesofsection(index).subscribe(res=>{
    var lst = res[0]['data'];
    for (let m=0;m<lst.length;m++){
      this.allque.push(lst[m]['ref'])
    }
  ...
  ques_data(){
    this.reportservice.selectedque = this.selectedque;
  // for 1st que:
  this.reportservice.selectedque_q1(this.index).subscribe(res=>{
    this.stats1 = res;
  ...
  // for 2nd que:
  this.reportservice.selectedque_q2(this.index).subscribe(res=>{
    this.stats2 = res;
...
// for 3rd que:
this.reportservice.selectedque_q3(this.index).subscribe(res=>{
  this.stats3 = res;
